...
public class MSWordWriter
{
    ...
    throws Throwable
  {
    for (int i = 0; i < paramArrayOfNavicatSQL.length; i++)
    {
      ...
      {
        org.ccs.sandbox.sqltool.datamodel.mysql.Field localField = arrayOfField[m];
        localVector.add("" + (m + 1));
        localVector.add(localField.getFieldName());
        localVector.add(localField.getFieldDesc().getMysqlTypeAndLength());
        localVector.add(localField.getFieldDesc().isNullable() ? "null" : "not null");
...
/* Location:              D:\DataDictionaryTool_0.2.1beta with dependencies\DataDictionaryTool_0.2.1beta.jar!\org\ccs\sandbox\sqltool\output\MSWordWriter.class
